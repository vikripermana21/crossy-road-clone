  ...
  generateRows(iteration) {
    ...
    new Array(iteration).fill(0).forEach((_, index) => {
      ...
      if (randomType === TYPE.FOREST) {
        const trees = new Array(THREE.MathUtils.randInt(0, 4))
          .fill(0)
          .map((_, index) => {
            return {
  ...
  animateVehicles() {
    METADATA.forEach((item) => {
      ...
      item.vehicles.forEach(({ ref }) => {
        ...
        if (item.direction) {
          ref.instance.position.x =
            ref.instance.position.x > endOfRow
              ? beginningOfRow
              : ref.instance.position.x + item.speed * this.time.delta * 0.0007;
        } else {
          ref.instance.position.x =
            ref.instance.position.x < beginningOfRow
              ? endOfRow
              : ref.instance.position.x - item.speed * this.time.delta * 0.0007;
        }
      });
